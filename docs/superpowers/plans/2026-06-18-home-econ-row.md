...
- ESP-IDF 명령 전 매 셸에서 `source "~/.espressif/tools/activate_idf_v6.0.1.sh"` 선행. `idf.py`는 저장소(워크트리) 루트 `~/Documents/stock_esp32/.claude/worktrees/fix+moreclock` 에서 실행.
...
```bash
cd ~/Documents/stock_esp32/.claude/worktrees/fix+moreclock/components/stock_core/test/host
...
```bash
cd ~/Documents/stock_esp32/.claude/worktrees/fix+moreclock
...
```bash
cd ~/Documents/stock_esp32/.claude/worktrees/fix+moreclock/sim
