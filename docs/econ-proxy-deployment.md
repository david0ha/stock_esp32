...
### 7-2. 전용 터널 만들기 (another self-hosted service와 동일한 named-tunnel 패턴)
...
# 3) 로컬 ingress 설정 (another self-hosted service 터널과 같은 config 방식)
...
- 보유 도메인(`example.com`/`example.org` 등)의 **서브도메인은 Cloudflare
...
### 9-1. stock_esp32 전용 러너 등록 (기존 another self-hosted service 러너와 별도)
...
### 9-2. repo secret 설정
```bash
gh secret set ECON_PROXY_TOKEN --repo youruser/stock_esp32   # 프록시 토큰과 동일 값
```

러너가 `docker compose`를 쓰므로 러너 실행 계정(deploy)이 `docker` 그룹에 있어야 한다
(`sudo usermod -aG docker deploy` 후 재로그인). 이후 main 푸시 → 자동 빌드/배포.
